...
import icons from './img/food/icons.png'
import { useDispatch, useSelector } from 'react-redux';
import {setActiveCat, setCategoryTitle} from '../../actions.js';
...
const InfoPage = () => {
  ...
  useEffect(() => {
    dispatch(setActiveCat(activeCategory)); // Установка активной категории при загрузке страницы
    console.log("Active category:", activeCategory); // Добавьте эту строку для отслеживания значения activeCategory
  }, [activeCategory]);
  const handleCategoryClick = async (categoryId) => {
    ...
    dispatch(setActiveCat(categoryId))
  ...
  useEffect(() => {
    dispatch(setActiveCat(activeCategory))
  }, [])
          ...
          {showFilterPage && (
              <div className={cl.filterPageOverlay}>
                <div className={cl.modalContainer} onClick={handleFilterPageClose}>
                  <FilterPage activeCategory={actCat} handleFilterPageClose={handleFilterPageClose} />
                </div>
              </div>
          )}
          ...
          <section className={`${cl.page__food} ${cl.food}`}>
            ...
            <div className={`${cl.food__header} ${cl._container}`}>
              ...
              <div className={cl.food__icon}>
                <img onClick={loopClick} src={icons} alt=""/>
              </div>
            </div>
